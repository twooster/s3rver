...
class FilesystemStore {
  ...
  async putObjectMultipart(bucket, uploadId, parts) {
    ...
    const partStreams = sortedParts.map(part =>
    ...
    const md5Streams = concatStreams(sortedParts.map(part =>
      fs.createReadStream(path.join(uploadDir, `${part.number}.md5`))
    ));
    const md5 = await new Promise((resolve, reject) => {
      const md5Context = crypto.createHash("md5");
      md5Streams
        .on("data", chunk => md5Context.update(chunk.toString(), "hex"))
        .on("error", reject)
        .on("end", () => resolve(md5Context.digest("hex") + "-" + parts.length));
    });
